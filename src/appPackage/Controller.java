...
public class Controller {
    ...
    public static final AudioClip audioMenu = new AudioClip(new File("src/subwooferLullaby.mp3").toURI().toString());

    public void playGame(ActionEvent e) throws IOException {
        if (audioMenu.isPlaying()){
            audioMenu.stop();
        }
        // ***** main game loading method *****
    ...
    public void startNewGame(ActionEvent e) throws IOException {
        ...
        stage.show();
        if (Player.getAudioWin().isPlaying()){
            Player.getAudioWin().stop();
        }
    }
